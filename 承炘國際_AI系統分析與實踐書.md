...
第一階段完全依《成本費用分析表》執行：沿用現有電腦時投入 NT$60,000–80,000；客戶自行購買簡易小型電腦及首月工具後，總額仍不超過 NT$100,000；每月工具費目標 NT$500–3,500，時程 4–6 週。初期只做一個產品、一個國家、一個信箱及一位操作人員，資料放在 SQLite，以 CSV／Excel 或簡易本機頁面操作。
...
2. 一個低價自動來源、一個 CSV 補充來源、公司官網核對、去重、評分及 Email 驗證。
3. 建立 300 筆候選公司，前 20 封人工校正後才允許規則自動寄送。
...
| ID | 需求 |
|---|---|
| FR-10 | 初期可從一個低價自動來源及一個人工 CSV 補充來源建立潛客 |
...
下表是可使用的來源池，不代表初期全部串接。10 萬元內版本只實作一個低價自動來源，另支援一個由承炘合法取得後匯入的 CSV 補充來源；Panjiva、Kompass 等付費合約須先證明名單有效，再另案採購。
...
第一輪選一個產品、單一國家，由一個低價自動來源加上一個合法 CSV／公開名錄補充來源取得 300 筆候選公司。來源不足時允許延長蒐集或調整搜尋條件，不為湊數購買昂貴年約，也不降低寄送門檻：
...
- **名單來源**：初期只做一個低價自動來源及一個 CSV 補充來源；其他來源保留相同匯入格式。
...
本案屬於**有條件可行**，可在 4–6 週內以「單一市場、單一寄件信箱、300 筆候選公司」完成本機初期版。現有系統介面可完成寄信、增量查信、AI 分類及本機商機建檔；主要不確定性不在 AI，而在企業帳號權限、資料來源品質、Email 驗證效果及寄件網域信譽。CRM、雲端與多人介面不影響初期技術成立。

| 子系統 | 可行性 | 建議實作 | 已知限制／依賴 | 驗證證據 |
|---|---|---|---|---|
| 開發活動、規則與流程狀態 | 高 | 自建工作流與資料庫；AI 不得直接改規則 | 需先定義目標客戶條件、頻率、停止條件 | 同一輸入可重現相同放行結果 |
| 從零建立公司名單 | 中高 | 一個低價自動來源＋一個 CSV 補充來源＋允許讀取的官網 | 需產品關鍵字、HS Code、目標國家與來源使用權利 | 300 筆候選公司的來源、官網配對與適配抽查達標 |
...
| G2 名單蒐集與去重 | 一個低價自動來源＋一個 CSV 補充來源建立 300 筆候選公司 | 來源 100% 可追溯；官網配對 ≥95%；錯誤合併 0；高風險 Email 全攔截 | 修正產品詞、HS Code、國家或更換來源；不先買年約 |
...
初期採 4–6 週本機實作，每個付款點完成驗收後才進下一段；名單關卡未通過時，不繼續支付寄信與回覆功能。

| 週次／付款關卡 | 累計投入 | 工作與交付 | 驗收 |
|---|---:|---|---|
| 第 1 週／A | NT$10,000 | 一個產品、第一個國家、關鍵字、HS Code、客戶類型、目標職位、產品事實與寄送政策 | 承炘確認搜尋條件與抽查方法 |
| 第 2–3 週／B | NT$25,000 | 一個低價自動來源、一個 CSV 補充來源、官網讀取、來源證據、去重、評分及 300 筆候選公司 | 來源可追溯；禁止來源為 0；官網配對 ≥95%；適配抽查 ≥80% |
| 第 4 週／C | NT$45,000 | Email 查找／驗證、個人化、政策檢查、排程、最多兩次跟進、停止名單 | 前 20 封公司、姓名與產品事實錯誤為 0；不重複寄送 |
| 第 5–6 週／D | NT$60,000–100,000 | 每 5–10 分鐘增量查信、回覆分類、本機商機通知、執行紀錄、備份、操作教學；NT$60,000–80,000 為開發費，上限含選配電腦與工具 | 拒絕／退訂／硬退信 100% 停止；正向回覆正確交接；重開機可續跑 |
...
- 審查郵件權限、本機備份、資料保留、API 成本與供應商狀態；加購 CRM 後才審查 CRM 權限。
...
2. 指定一位業務主管與一位系統窗口。
3. 選定一個低價自動公司來源、一個合法 CSV／公開名錄補充來源及 Email 查找／驗證服務；初期不簽昂貴資料年約。
